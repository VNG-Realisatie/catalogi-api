...
class HistoryModelUserStoryTests(APITestCase):
    ...
    def get_zaaktype_list(self):
        ...
        data_zaaktype_2 = response.json()["results"][0]
        self.assertEqual(
            data_zaaktype_2["besluittypen"][0],
    ...
    def test_user_story_new_version_model_different_order(self):
        """
        In this userstory the following aspects of the new version model are tested:
        1. POST besluittypen with a zaaktypen array consisted of zaaktypen_identificatie (which is conventially an array of URI's)
        2. POST zaaktype with a besluittypen array consisted of besluittypen_omschrijvingen (which is conventially an array of URI's)
        3. GET a specific besluittype which contains a list of associated besluittypen. Only the most recent and concept = False besluittypen should be shown.
        """
        self.post_informatieobjecttype()
        self.post_zaaktype_1()
        self.post_besluittype_1()
