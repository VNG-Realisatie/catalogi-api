...
1. ``ztc-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``ztc-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``ztc-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``ztc-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``ztc-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``ztc-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``ztc-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``ztc-pr-isort``: checks that imports are correctly
